...
def two_pass_labeling(B):
            ...
            if B[row, col] != 0:
                ...
                for i in n:
                    if i is not None:
                        ...
                        if lb != m:
                            union(m, lb, linked)
    # сохраяем все лейблы в словарь
    labels_map = {}
    for row in range(B.shape[0]):
        for col in range(B.shape[1]):
            if B[row, col] != 0:
                new_label = find(labels[row, col], linked)
                if not (new_label in labels_map):
                    labels_map[new_label] = new_label
    # убираем пропуски между метками
    i = 1
    ...
    for row in range(B.shape[0]):
        for col in range(B.shape[1]):
            if B[row, col] != 0:
                new_label = find(labels[row, col], linked)
                normal_label = labels_map[new_label] # для маркировки используем соответствующею порядковую метку
                if normal_label != labels[row, col]:
                    labels[row, col] =  normal_label
    return labels
